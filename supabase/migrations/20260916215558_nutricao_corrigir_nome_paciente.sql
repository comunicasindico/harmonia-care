...
begin
 ...
 if p_action='patients' then
  select coalesce(jsonb_agg(jsonb_build_object('id',id,'empresa_id',empresa_id,'nome_completo',nome_completo,'nome_apelido',nome_completo,'data_nascimento',data_nascimento,'ativo',ativo) order by nome_completo),'[]') into result from public.pacientes where empresa_id=u.empresa_id;
  return result;
 end if;
