...
begin
 ...
 if p_action='administer' then
  ...
  if day_value is null or day_value>today then raise exception 'Não é permitido registrar uma administração futura.';end if;
  if day_value<today and (u.hierarquia is distinct from 1 or reason is null or coalesce((p_payload->>'retroativo')::boolean,false) is not true) then raise exception 'Registro retroativo exige nível 1, confirmação e justificativa.' using errcode='42501';end if;
  if old_r.ativo is not true or coalesce(old_r.status_tratamento,'ativo')<>'ativo' or old_r.data_inicio>day_value or old_r.data_fim<day_value
  ...
  insert into harmonia_med_private.audit(medicacao_id,empresa_id,usuario_id,acao,motivo,antes,depois) values(old_r.id,u.empresa_id,u.id,'administracao',reason,case when old_e.id is null then null else to_jsonb(old_e) end,to_jsonb(e));
  return to_jsonb(e);
 end if;
